...
for d = 1:depth
    ...
    % Extract weights
    wlen = double(netconfig.layersizes{d});
    stack{d}.w = reshape(params(curPos:curPos+wlen-1), netconfig.layerInfo{d}.wsize);
    curPos = curPos+wlen;

    % Extract bias
    blen = double(netconfig.layerInfo{d}.wsize(4));
    stack{d}.b = reshape(params(curPos:curPos+blen-1), netconfig.layerInfo{d}.wsize(4), 1);
    curPos = curPos+blen;
